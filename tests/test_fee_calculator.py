...
class TestFeeCalculatorPix:
    ...
    def test_pix_zero_fee(self, fee_calculator):
        fee = fee_calculator.calculate(Decimal("150.00"), PaymentMethod.PIX, 1)
        assert fee == Decimal("0.00")

    def test_pix_high_value_still_zero(self, fee_calculator):
        fee = fee_calculator.calculate(Decimal("99999.99"), PaymentMethod.PIX, 1)
        assert fee == Decimal("0.00")
...
class TestFeeCalculatorCard:
    ...
    def test_card_1x(self, fee_calculator):
        # 100.00 * 3.99% = 3.99
        fee = fee_calculator.calculate(Decimal("100.00"), PaymentMethod.CARD, 1)
        assert fee == Decimal("3.99")

    def test_card_2x(self, fee_calculator):
        # 100.00 * (4.99% + 2%) = 6.99
        fee = fee_calculator.calculate(Decimal("100.00"), PaymentMethod.CARD, 2)
    ...
    def test_card_3x_example_from_spec(self, fee_calculator):
        ...
        fee = fee_calculator.calculate(Decimal("297.00"), PaymentMethod.CARD, 3)
        assert fee == Decimal("26.70")

    def test_card_12x(self, fee_calculator):
        # 100.00 * (4.99% + 22%) = 26.99
        fee = fee_calculator.calculate(Decimal("100.00"), PaymentMethod.CARD, 12)
    ...
    def test_card_rounding_half_up(self, fee_calculator):
        ...
        fee = fee_calculator.calculate(Decimal("1.00"), PaymentMethod.CARD, 1)
